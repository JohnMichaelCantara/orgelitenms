...
const Register: React.FC<RegisterProps> = ({ users, onRegister, onSwitchToLogin }) => {
  ...
  const handleVerifyAndComplete = (e: React.FormEvent) => {
    ...
    if (otp.trim() === generatedOtp && generatedOtp !== '') {
      ...
      // Attempt registration - will be handled by App.tsx's non-blocking dbAction
      setTimeout(() => {
        onRegister(newUser);
      }, 500);
    } else {
    ...
    <div className="max-w-xl mx-auto mt-12 p-10 bg-white rounded-[4rem] shadow-2xl border border-slate-100 animate-in fade-in slide-in-from-bottom-6 duration-700">
      ...
      <div className="text-center mb-10 px-6">
        <div className="flex justify-center items-center -space-x-3 mb-8">
          {LOGO_URLS.map((url, i) => (
            <div key={i} className="w-16 h-16 bg-white rounded-2xl p-1 shadow-lg border border-slate-100 overflow-hidden transform transition-all hover:scale-110 hover:z-10 hover:-translate-y-1 cursor-default" style={{ transform: `rotate(${i === 0 ? '-8deg' : i === 2 ? '8deg' : '0deg'})` }}>
              <img src={url} alt={`Logo ${i + 1}`} referrerPolicy="no-referrer" className="w-full h-full object-contain" onError={(e) => { (e.target as HTMLImageElement).src = `https://api.dicebear.com/7.x/initials/svg?seed=${fallbacks[i]}&backgroundColor=f1f5f9&textColor=475569&fontSize=40`; }} />
            </div>
          ))}
        </div>
        <h2 className="text-4xl font-black text-slate-900 tracking-tight leading-none mb-4">Register Phone</h2>
        <p className="text-slate-400 font-medium leading-relaxed">Your mobile number is your identity. Verification ensures secure access to the community portal.</p>
      </div>
      ...
      {step === 1 ? (
        <form onSubmit={handleStartVerification} className="space-y-8">
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input type="text" required autoFocus value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} className="w-full pl-16 pr-6 py-5 bg-slate-50 border border-slate-100 rounded-[2.2rem] focus:ring-4 focus:ring-sky-100 outline-none font-bold text-lg transition-all" placeholder="Juan dela Cruz" />
              </div>
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-2">Mobile Key</label>
              <div className="relative group">
                ...
                <input type="tel" required value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value.replace(/\D/g, '').slice(0, 10) })} className="w-full pl-24 pr-6 py-5 bg-slate-50 border border-slate-100 rounded-[2.2rem] focus:ring-4 focus:ring-sky-100 outline-none font-bold text-lg transition-all" placeholder="917 123 4567" />
              </div>
            </div>
          </div>
          <button type="submit" disabled={isSending} className="w-full py-6 bg-slate-900 text-white rounded-[2.2rem] font-black text-lg hover:bg-slate-800 transition-all shadow-2xl flex items-center justify-center gap-4 active:scale-95 disabled:opacity-50">
            {isSending ? <><Loader2 className="w-6 h-6 animate-spin" /> Verifying Device...</> : <>Register & Get Code <ArrowRight className="w-6 h-6" /></>}
          </button>
        </form>
      ) : (
        <form onSubmit={handleVerifyAndComplete} className="space-y-10">
          ...
          <button type="submit" disabled={isFinishing} className="w-full py-6 bg-sky-600 text-white rounded-[2.2rem] font-black text-lg shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-4 disabled:opacity-50">
            {isFinishing ? <><Loader2 className="w-6 h-6 animate-spin" /> Creating Profile...</> : <>Finalize My Portal Account</>}
          </button>
        </form>
      )}
    </div>
