...
import { 
  ...
  Compass, Sparkles, Check, X, Users
} from 'lucide-react';
    ...
    <div className="space-y-8 lg:space-y-12 animate-in fade-in slide-in-from-bottom-10 duration-700">
      <section className="relative overflow-hidden gradient-pastel rounded-[2.5rem] lg:rounded-[4rem] p-8 lg:p-24 text-slate-900 shadow-2xl shadow-emerald-100/50">
        <div className="relative z-10 max-w-2xl">
          ...
          <p className="text-base lg:text-xl text-slate-700 mb-10 max-w-md font-medium leading-relaxed opacity-80">
            The portal is now active. As a member of our community, you can explore resources, track events, and stay informed.
          </p>
          <div className="flex flex-wrap gap-4">
            <button onClick={onExplore} className="px-8 lg:px-12 py-4 lg:py-5 bg-slate-900 text-white rounded-2xl lg:rounded-[2rem] font-black text-sm lg:text-lg flex items-center gap-2 transform transition-all hover:-translate-y-1 hover:shadow-2xl hover:shadow-slate-500/20 active:scale-95">
              Explore Resources <ArrowRight className="w-4 h-4 lg:w-6 h-6" />
            </button>
            {isAdmin && (
              <button onClick={onManage} className="px-8 lg:px-12 py-4 lg:py-5 bg-white/40 text-slate-900 border border-white/60 rounded-2xl lg:rounded-[2rem] font-black text-sm lg:text-lg backdrop-blur-md transition-all hover:bg-white/70 hover:shadow-xl active:scale-95">
                Admin Center
              </button>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
        <div className="lg:col-span-2 space-y-8 lg:space-y-12">
          {isAdmin && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
              <button onClick={handleQuickAnnounce} className="p-6 lg:p-10 bg-white rounded-3xl border border-slate-100 shadow-sm flex items-center gap-6 transition-all hover:shadow-xl hover:border-sky-100 group">
                ...
                <div className="text-left">
                  ...
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Post to feed</p>
                </div>
              </button>
              <button onClick={handleLocalUpload} className="p-6 lg:p-10 bg-white rounded-3xl border border-slate-100 shadow-sm flex items-center gap-6 transition-all hover:shadow-xl hover:border-emerald-100 group">
                ...
                <div className="text-left">
                  <h3 className="text-lg lg:text-xl font-black text-slate-800">Add Resource</h3>
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Secure share</p>
                </div>
              </button>
            </div>
          )}
          ...
          <section>
            <h2 className="text-2xl lg:text-3xl font-black text-slate-900 mb-6 lg:mb-8">Latest Bulletins</h2>
            {announcements.length === 0 ? (
              <div className="bg-white py-16 lg:py-24 rounded-[2.5rem] lg:rounded-[3.5rem] border border-slate-100 text-center shadow-sm flex flex-col items-center gap-4">
                ...
                <div>
                  <p className="text-slate-400 font-black uppercase tracking-[0.2em] text-xs">Waiting for Updates</p>
                  <p className="text-slate-300 text-[10px] mt-1">Official bulletins will appear here</p>
                </div>
              </div>
            ) : (
                  ...
                  <div key={ann.id} id={`ann-${ann.id}`} className="bg-white p-6 lg:p-10 rounded-3xl lg:rounded-[3rem] border border-slate-100 shadow-sm transition-all hover:border-sky-50 scroll-mt-24">
                    <div className="flex items-start gap-4 lg:gap-6">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm lg:text-base text-slate-600 leading-relaxed line-clamp-2">{ann.content}</p>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <div className="space-y-8 lg:space-y-12">
          <section>
            ...
            {events.length === 0 ? (
              <div className="p-10 bg-white/50 rounded-[2.5rem] border border-dashed border-slate-200 text-center flex flex-col items-center gap-3">
                ...
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Calendar Empty</p>
              </div>
            ) : (
                    ...
                    <div key={event.id} id={`evt-${event.id}`} className="p-4 lg:p-6 bg-white rounded-2xl lg:rounded-3xl border border-slate-100 shadow-sm transition-all hover:border-sky-50 scroll-mt-24">
                      <div className="flex items-center justify-between gap-4">
                        <div className="flex items-center gap-4 overflow-hidden">
                          <div className={`w-10 h-10 lg:w-12 lg:h-12 ${isJoined ? 'bg-emerald-50 text-emerald-600' : 'bg-sky-50 text-sky-600'} rounded-xl flex items-center justify-center shrink-0`}>
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-2">
                                <p className="text-[10px] font-bold text-slate-400 uppercase">{new Date(event.date).toDateString()}</p>
                        ...
                        <button 
                          ...
                          className={`p-2 lg:p-3 rounded-xl transition-all ${
                            isJoined 
                        ...
                        >
                          {isJoined ? (
                            ...
                            <div className="flex items-center gap-1 px-1">
                              <span className="text-[10px] font-black uppercase tracking-tight">Join Event</span>
                            </div>
                          )}
                        </button>
                      </div>
          ...
          <section>
            <h2 className="text-xl lg:text-2xl font-black text-slate-800 mb-6">Recent Assets</h2>
            <div className="grid grid-cols-2 lg:grid-cols-2 gap-3 lg:gap-4">
              {gallery.length === 0 ? (
                ...
                gallery.slice(0, 4).map(item => (
                  <div key={item.id} className="aspect-square bg-white rounded-2xl border border-slate-100 overflow-hidden relative shadow-sm transition-transform hover:scale-105">
                    {item.type === 'PHOTO' ? (
            ...
            {gallery.length === 0 && (
              <p className="text-center mt-4 text-[9px] font-black text-slate-300 uppercase tracking-widest">No shared resources yet</p>
            )}
          </section>
        </div>
      </div>
